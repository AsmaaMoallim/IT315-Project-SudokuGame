...
w = [[0 for x in range(9)] for y in range(9)]
        ...
        for col in range(9):
            ...
            for i in range(row_x, row_x+3):
                for j in range(col_y, col_y+3):
                    print('check box: row,col,(i,j)', row,
                          ',', col, ',(', i, ',', j, ')')
...
def sudoku():
    ...
    main_window = Tk()
    sudoku_font = tkFont.Font(size=15, weight="bold")
    by_font = tkFont.Font(size=10)
    button_text_font = tkFont.Font(size=10, weight="bold")
    ...
    lable_by = Label(
        top_frame, text='By: Asmaa & Rinad & Rahaf  ^_^  ', font=by_font, fg='Blue')
            ...
            for j in range(9):
                ...
                if(x[i][j] == 0):
                    ...
                    if(j == 2 or j == 5):
                        va_padx = 13
                    sudoku_grid = Entry(
                        mid_frame, width=3, bg='light blue', fg='black', justify='center')
                    sudoku_grid.grid(row=i, column=j, padx=(
                        0, va_padx), pady=(0, va_pady), ipadx=3, ipady=3)
                    if (w[i][j] != 0):
                        sudoku_grid_lable = Label(
                            mid_frame, text=lis[i][j], width=3, bg='light blue', fg='black', font=button_text_font)
                        sudoku_grid_lable.grid(row=i, column=j, padx=(
                            0, va_padx), pady=(0, va_pady), ipadx=0, ipady=2)
        ...
        for i in range(9):
            for j in range(9):
                ...
                if (w[i][j] != 0):
                    ...
                    sudoku_grid = Entry(mid_frame, width=3, bg='light blue',
                                        fg='black', textvariable=x_filed, justify='center')
                    sudoku_grid.grid(row=i, column=j, padx=(
                        0, va_padx), pady=(0, va_pady), ipadx=3, ipady=3)
...
# choose random numbers
        ...
        for N in range(generate):
            ...
            col = randrange(9)
            num = randrange(1, 10)
            # Fill the position if not taken or not valid
            while (not valid(lis, (row, col),  num) or lis[row][col] != 0):
                row = randrange(9)
                col = randrange(9)
                num = randrange(1, 10)
            lis[row][col] = num
...
# fill the random numbers
        ...
        for i in range(9):
            for j in range(9):
                ...
                if (lis[i][j] != 0):
                    sudoku_grid_lable = Label(
                        mid_frame, text=lis[i][j], width=5, bg='light blue', fg='black', font=button_text_font)
                    sudoku_grid_lable.grid(row=i, column=j, padx=(
                        0, va_padx), pady=(0, va_pady), ipadx=0, ipady=2)
                elif(lis[i][j] == 0):
                    sudoku_grid = Entry(
...
In Case You Didn\'t, Then Well Done Genius ^_^ ^_^')
    ...
    Check_button = Button(bottom_frame, text='Check', width=70, bg='blue',
                          fg='White', font=button_text_font, borderless=1, command=Response)
    StartNew_button = Button(bottom_frame, text='Start New', width=70, bg='blue',
                             fg='White', font=button_text_font, borderless=1, command=Start_New)
    Clear_button = Button(bottom_frame, text='Clear', width=70, bg='blue',
                          fg='White', font=button_text_font, borderless=1, command=Clear_input)
    Solve_button = Button(bottom_frame, text='Solve', width=70, bg='blue',
                          fg='White', font=button_text_font, borderless=1, command=so)
